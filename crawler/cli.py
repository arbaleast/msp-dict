...
def cmd_fetch(args):
    ...
    if args.github or args.all:
        print(f"\n[{index}/{TOTAL_SOURCES}] Fetching GitHub Trending...")
    ...
    if args.wikipedia or args.all:
        print(f"\n[{index}/{TOTAL_SOURCES}] Fetching Wikipedia Most Visited...")
    ...
    if args.bilibili or args.all:
        print(f"\n[{index}/{TOTAL_SOURCES}] Fetching Bilibili Hot Keywords...")
    ...
    # Moegirl Wiki
    if args.moegirl or args.all:
        print(f"\n[{index}/{TOTAL_SOURCES}] Fetching Moegirl Wiki...")
        fetcher = MoegirlFetcher(proxy=args.proxy)
        titles = fetcher.fetch(limit=args.limit)
        sources['moegirl'] = titles
        print(f"      Got {len(titles)} article titles")
        index += 1

    # Wikidata
    if args.wikidata or args.all:
        print(f"\n[{index}/{TOTAL_SOURCES}] Fetching Wikidata...")
        fetcher = WikidataFetcher(proxy=args.proxy)
        labels = fetcher.fetch(limit=args.limit)
        sources['wikidata'] = labels
...
def cmd_stats(args):
    ...
    if args.moegirl or args.all:
        print("\n[Moegirl Wiki]")
        print("  API: zh.moegirl.org.cn/api.php")
        fetcher = MoegirlFetcher(proxy=args.proxy)
        titles = fetcher.fetch(limit=100)
        print(f"  Current count: {len(titles)}")
        stats_info.append(('Moegirl', len(titles)))

    # Wikidata
    if args.wikidata or args.all:
        print("\n[Wikidata]")
        print("  API: www.wikidata.org/w/api.php")
        fetcher = WikidataFetcher(proxy=args.proxy)
        labels = fetcher.fetch(limit=100)
...
Examples:
  ...
  # 只抓取萌娘百科和Wikidata
  python -m crawler.cli fetch --moegirl --wikidata
